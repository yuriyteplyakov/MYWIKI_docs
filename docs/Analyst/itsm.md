# Все об ITSM

## ITSM (IT Service Management)

это подход к управлению IT-сервисами, который описывает процессы обработки инцидентов, запросов пользователей и изменений системы.

## Основная методология — ITIL.

ITIL (Information Technology Infrastructure Library) — это ведущая международная библиотека лучших практик (фреймворк) для управления IT-услугами (ITSM), помогающая компаниям согласовывать ИТ-процессы с бизнес-стратегией. Она обеспечивает максимальную эффективность, прозрачность и создание ценности для клиентов, охватывая все этапы от проектирования до поддержки.

### Основные характеристики и цели ITIL:

Ориентация на бизнес: ИТ рассматриваются не как техническая поддержка, а как услуги, приносящие ценность бизнесу.

Структура (ITIL 4): Современная версия (ITIL 4) ориентирована на гибкость, цифровизацию и включает 34 практики, объединяющие процессы, людей и технологии.
Основные этапы (Жизненный цикл, ITIL v3/4): Стратегия, Проектирование, Внедрение, Эксплуатация (Service Operation) и Непрерывное улучшение услуг.

**Цель:** Оптимизация ресурсов, повышение качества обслуживания пользователей и снижение рисков.

**ITIL часто путают с ITSM (IT Service Management):**
ITIL — это набор рекомендаций (теория), а ITSM — это подход, реализующий эти рекомендации на практике.

1C-Connect
корпоративная почта
CRM
Service Desk
ERP


## CMDB (Configuration Management Database — База данных управления конфигурациями)

> это централизованное хранилище информации обо всех компонентах ИТ-инфраструктуры (конфигурационных единицах: серверах, ПО, сетях, сервисах) и, что критически важно, взаимосвязях между ними. Она обеспечивает полную картину ИТ-ландшафта для эффективного управления инцидентами, изменениями и рисками. 

**Основные характеристики CMDB:**

> Конфигурационные единицы (CI): Включает не только «железо» (серверы, ноутбуки), но и «софт», облачные сервисы, документацию и людей.
Взаимосвязи: Показывает, как один компонент влияет на другой. Например, как падение конкретного сервера повлияет на работу веб-сайта компании.
Актуальность: Регулярно обновляется (часто автоматически через инструменты Discovery) для отражения текущего состояния. 

**Зачем нужна CMDB:**

> Быстрое устранение инцидентов: Позволяет видеть корень проблемы (root cause analysis), понимая зависимость сервисов от инфраструктуры.
Управление изменениями: Помогает оценить риски перед обновлением оборудования или ПО, предвидя, что может сломаться.
Оптимизация активов (ITAM): Дает точные данные для инвентаризации, планирования закупок и управления лицензиями. 

> CMDB является ядром процессов ITSM (управления ИТ-услугами) и помогает перейти от реактивного управления к проактивному.

![1](images/1.jpg)

![2](images/2.jpg)

![3](images/3.jpg)

![4](images/4.jpg)

![5](images/5.jpg)

![6](images/6.jpg)

![7](images/7.jpg)

![8](images/8.jpg)

![9](images/9.jpg)

![10](images/10.jpg)

![11](images/11.jpg)

![12](images/12.jpg)

![13](images/13.jpg)

![14](images/14.jpg)

![15](images/15.jpg)

![16](images/16.jpg)

![17](images/17.jpg)

![18](images/18.jpg)

![19](images/19.jpg)

![20](images/20.jpg)

![21](images/21.jpg)

![22](images/22.jpg)